...
def _build_nusc_sensor_meta(info, frame_key: str, affine_matrix: np.ndarray):
...
class nuScenes_range_image(data.Dataset):
    def __init__(self,
        ...
        if manifest_by_original_index is not None:
            self.cache_entries = []
            for original_index, info in self.samples:
...
def build_frame_mapping(data, dataset_key, frame_key, depth_map,
                        affine_matrix, idx=None, dataset_root='./Datasets/nuscenes',
...
def build_frame_mapping_fast(
    ...
    if dataset_key == 'sjtu':
        for ch in camera_channels:
            K     = np.asarray(sample_info[f'sensor_metas_{frame_key}'][ch]['K'], dtype=np.float32)
            R_l2c = np.asarray(sample_info[f'sensor_metas_{frame_key}'][ch]['R_l2c'], dtype=np.float32)
            t_l2c = np.asarray(sample_info[f'sensor_metas_{frame_key}'][ch]['t_l2c'], dtype=np.float32).reshape(3, 1)
