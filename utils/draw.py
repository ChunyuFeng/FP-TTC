...
def scale2rgb(scale):
    ...
    # ], dtype=np.float32)
    colors = np.array([
        [0.0, 0.0, 0.0],   # 黑 Black        @ 0.0    # TTC = 0
        [1.0, 1.0, 0.0],   # 黄 Yellow       @ 0.9    # TTC = 1
        [0.0, 0.0, 1.0],   # 蓝 Blue         @ 0.9667 # TTC = 3
        [0.0, 1.0, 1.0],   # 青/青色 Cyan     @ 0.98   # TTC = 5
        [1.0, 0.0, 1.0],   # 品红 Magenta     @ 1.0    # TTC = ∞
        [0.0, 1.0, 0.0],   # 绿 Green         @ 1.02   # TTC = -5（远离）
        [1.0, 0.0, 0.0],   # 红 Red           @ 1.0333 # TTC = -3（远离）
        [1.0, 1.0, 1.0],   # 白 White         @ 1.1    # TTC = -1（远离）
        [1.0, 1.0, 1.0],   # 白 White         @ 2.0    # 末段保持白色（远离）
    ], dtype=np.float32)
