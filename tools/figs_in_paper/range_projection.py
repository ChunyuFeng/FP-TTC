...
def load_lidar_xyz_from_sample_data(nusc, sd):
    """从 NuScenes sample_data 读取 LIDAR_TOP 点云 (N,3)"""
    lidar_path = os.path.join(nusc.dataroot, sd['filename'])
    arr = np.fromfile(lidar_path, dtype=np.float32).reshape(-1, 5)[..., :4]
    return arr[:, :3]
...
def main(args):
    ...
    for i in tqdm(range(1, len(camera_lut)), desc="Combining camera and scene flow data"):
        ...
        # ========= 新增：对 LIDAR_TOP 原始点云做 range projection 并保存 =========
        # 加载 prev / curr 两帧的 LIDAR_TOP 原始点云 (N,3)
        lidar_prev_xyz = load_lidar_xyz_from_sample_data(nusc, lidar_data_matched_with_images[i-1])
        lidar_curr_xyz = load_lidar_xyz_from_sample_data(nusc, lidar_data_matched_with_images[i])

        # 占位的 scale / risk（与已有保存结构键一致，便于下游共用）
        scales_prev_placeholder = np.ones(lidar_prev_xyz.shape[0], dtype=np.float32)
        risk_prev_placeholder   = np.zeros(lidar_prev_xyz.shape[0], dtype=np.float32)

        scales_curr_placeholder = np.ones(lidar_curr_xyz.shape[0], dtype=np.float32)
        risk_curr_placeholder   = np.zeros(lidar_curr_xyz.shape[0], dtype=np.float32)
        ...
        (proj_range_l_prev,
         ...
         proj_xyz_l_prev,
         proj_idx_l_prev,
         proj_mask_l_prev) = range_projection(
            lidar_prev_xyz,
            scales_prev_placeholder,
            risk_prev_placeholder,
            H=args.image_size[0],
            W=args.image_size[1]*6,
            fov_up=args.fov[0],
            fov_down=-args.fov[1]
        ...
        (proj_range_l_curr,
         ...
         proj_xyz_l_curr,
         proj_idx_l_curr,
         proj_mask_l_curr) = range_projection(
            lidar_curr_xyz,
            scales_curr_placeholder,
            risk_curr_placeholder,
            H=args.image_size[0],
            W=args.image_size[1]*6,
            fov_up=args.fov[0],
            fov_down=-args.fov[1]
        ...
        # # 组织与现有一致的字典结构（键保持一致，方便 loader 直接复用）
        # range_image_lidar_prev = {
        #     'depth': proj_range_l_prev,
        #     'scale': proj_scale_l_prev,           # = 1 的占位
        #     'risk_score': proj_risk_l_prev,       # = 0 的占位
        #     'xyz': proj_xyz_l_prev,
        #     'idx': proj_idx_l_prev,
        #     'mask': proj_mask_l_prev
        # }
        # range_image_lidar_curr = {
        #     'depth': proj_range_l_curr,
        #     'scale': proj_scale_l_curr,
        #     'risk_score': proj_risk_l_curr,
        #     'xyz': proj_xyz_l_curr,
        #     'idx': proj_idx_l_curr,
        #     'mask': proj_mask_l_curr
        # }

        # # 保存到和 prev/curr range image 相同目录下
        # np.save(os.path.join(range_image_save_path, 'range_image_lidar_prev.npy'),
        #         range_image_lidar_prev)
        # np.save(os.path.join(range_image_save_path, 'range_image_lidar_curr.npy'),
        #         range_image_lidar_curr)
        # # ========= 新增结束 =========
        ...
        # ======================= 深度图可视化（proj_range_ 与 proj_range_l_curr） =======================
        if args.depth_map_vis:
            depth_map_vis_save_path = os.path.join(
                args.vis_dir, 'depth_map',
                f"{args.image_size[0]}_{args.image_size[1]*6}_fov_{args.fov[0]}_{args.fov[1]}"
            )
            if not os.path.exists(depth_map_vis_save_path):
                os.makedirs(depth_map_vis_save_path)

            def save_depth_vis(depth_map: np.ndarray, out_name: str):
                """
                depth_map: (H, W) 浮点深度，-1 表示无效
                可视化要求：越远越黑（magma 的黑端），越近越亮。
                """
                depth = np.copy(depth_map)
                valid = depth > 0  # 无效像素为 -1
                if np.any(valid):
                    vmin = float(np.percentile(depth[valid], 1.0))
                    vmax = float(np.percentile(depth[valid], 99.0))
                    if vmax <= vmin:
                    ...
                    # 先按近(小深度)->0，远(大深度)->1 归一化
                    norm_valid = (depth[valid] - vmin) / denom
                    norm_valid = np.clip(norm_valid, 0.0, 1.0)
                    # 再取反：远(1)->0(黑)，近(0)->1(亮)
                    norm[valid] = 1.0 - norm_valid
                else:
                    norm = np.zeros_like(depth, dtype=np.float32)

                out_path = os.path.join(depth_map_vis_save_path, out_name)
                plt.imsave(out_path, norm, cmap='magma')


            # 保存当前帧（scene flow 对应）的深度可视化
            save_depth_vis(
                proj_range_,  # points_curr 投影深度
                f"scene_{scene_indice}_dense_depth_{i}.png"
            )

            # 保存当前帧 LIDAR_TOP 原始点云的深度可视化
            # 注意：proj_range_l_curr 来自前面新增的 LIDAR 投影代码
            save_depth_vis(
                proj_range_l_curr,
                f"scene_{scene_indice}_sparse_depth_{i}.png"
            )
        # ======================= 深度图可视化结束 =======================
