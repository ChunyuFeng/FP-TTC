...
def generate_pseduo_point_cloud(nusc,
                                lidar_token,
                                surround_view_data,
                                projected_points,
                                use_metric_depth_estimator,
    ...
    lidar_points_supplement = {}
    plot_depth_analysis = False
    if use_metric_depth_estimator:
        ...
        metric_encoder = 'vitl' # or 'vits', 'vitb'
        dataset = 'vkitti' # 'hypersim' for indoor model, 'vkitti' for outdoor model
        max_depth = 80 # 20 for indoor model, 80 for outdoor model
        metric_depth_anything = Metric_DepthAnythingV2(**{**metric_model_configs[metric_encoder], 'max_depth': max_depth})
        metric_depth_anything.load_state_dict(torch.load(f'depthanything/metric_depth/checkpoints/depth_anything_v2_metric_{dataset}_{metric_encoder}.pth',
                                        map_location='cpu'))
        metric_depth_anything.to('cuda').eval()

        # TODO: 实现深度+图像生成伪激光点云
        for idx, channel in enumerate(projected_points):
            # 读取图像并进行深度推理
            raw_image = np.array(projected_points[channel]['original_img'])
            im=projected_points[channel]['original_img']
            im.save(f'{channel}.png')
        ...
        depth_anything.load_state_dict(torch.load(f'depthanything/checkpoints/depth_anything_v2_{encoder}.pth',map_location='cpu'))
        ...
        if plot_depth_analysis:
            ...
            for idx, channel in enumerate(projected_points):
                ...
                ax.plot(X.flatten()[sorted_indices], y_fit[sorted_indices],
                        color='green', linewidth=2, label='RANSAC fit')           
                ax.set_xlabel('depth_pred', fontsize=16)
                ax.set_ylabel('1/depth_meas', fontsize=16)
                ax.set_title(f'{channel}', fontsize=18)
                ax.grid(True)
                ax.legend(fontsize=12)
            # 调整子图布局
...
def project_surround_view_to_lidar(nusc,
    ...
    channel = cam_channel
    cam_sd = nusc.get('sample_data', surround_view_data[channel]['token'])
